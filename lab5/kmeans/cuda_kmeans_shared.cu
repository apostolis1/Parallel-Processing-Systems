                    ...
                    int    clusterId)
{
    ...
    for (i = 0; i < numCoords; i++)
        ans += (objects[objectId+i*numObjs]-clusters[clusterId+numClusters*i]) * (objects[objectId+i*numObjs]-clusters[clusterId+numClusters*i]);
                          ...
                          float *devdelta)
{
    ...
    int j;
    if (local_tid == 0) {
        // checkCuda(cudaMemcpy(shmemClusters, deviceClusters, numObjs*numCoords*sizeof(float), cudaMemcpyDeviceToDevice));
        for (j = 0; j < numClusters*numCoords; j++) {
            shmemClusters[j] = deviceClusters[j];
//            printf("Element %d of shmemClusters is : %f\n",j,shmemClusters[j]);
        }
    }
    __syncthreads();
						...
						int blockSize)  
{
    ...
    do {
		...
		//printf("Launching find_nearest_cluster Kernel with grid_size = %d, block_size = %d, shared_mem = %d \n", numClusterBlocks, numThreadsPerClusterBlock, clusterBlockSharedDataSize/sizeof(float));
