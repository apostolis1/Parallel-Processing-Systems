...
int main(int argc, char ** argv) {
    ...
    int i,j,t;
    int converged=0; //flags for convergence, global and per process
    MPI_Datatype dummy;     //dummy datatype used to align user-defined datatypes in memory
    double omega; 			//relaxation factor - useless for Jacobi

    struct timeval tts,ttf,tcs,tcf,tes,tef,tss,tsf;   //Timers: total-> tts,ttf, computation -> tcs,tcf
    double ttotal=0,tcomp=0,tcomm=0,tconv=0,total_time,comp_time;
    double *comm_time, *conv_time;
    int *global_converged;
    
    comm_time = (double *) malloc (sizeof(double));
    conv_time = (double *) malloc (sizeof(double));
    global_converged = (int *) malloc (sizeof(int));
    *global_converged = 0;
	...
	*/
    ...
    #ifdef TEST_CONV
    // for (t=0; t<T && *global_converged != 0;t++) {
    for (t=0;t<3201;t++) {
    ...
    for (t=0;t<T;t++) {
        ...
        //printf("Beginning of for t: %d tcomm is: %lf, for rank: %d\n", t, tcomm, rank);//
        // printf("\n");
        //*************TODO*******************//
        ...
        int counter_recv = 0, counter_send = 0;
        // Prepare buffers for send here, so we can measure only communication time later
        // Send north
        if (north != -1)     
            for (i = 0; i < local[1]; i++) 
        ...
        // Send before computation to north end west if they exist, t = T send t = T-1
        
        if (north != -1) {    
        // Send north
            MPI_Isend(up_send,local[1],MPI_DOUBLE,north,north,CART_COMM, &send_requests[counter_send]); 
            counter_send++;
        }
        if (west != -1) {
        // Send west 
            MPI_Isend(left_send,local[0],MPI_DOUBLE,west,west,CART_COMM, &send_requests[counter_send]);
            counter_send++;
        }

        // Similarly, receive from south end east if they exist
        if (south != -1) {
        //  Receive south
            MPI_Irecv(down_receive,local[1],MPI_DOUBLE,south,rank,CART_COMM, &receive_requests[counter_recv]);
            counter_recv++;
        }
        ...
        if (east != -1) {
        //   Receive east 
            MPI_Irecv(right_receive,local[0],MPI_DOUBLE,east,rank,CART_COMM, &receive_requests[counter_recv]);
            counter_recv++;
        }

        // First wait for send requests to complete, so the processes that only send (eg rank 0) can go past that point
        // and the other processes waiting for them can continue
        if (counter_send != 0)
            MPI_Waitall(counter_send,send_requests,MPI_STATUSES_IGNORE);               
        if (counter_recv != 0)
            MPI_Waitall(counter_recv,receive_requests,MPI_STATUSES_IGNORE);               
        ...
        //printf("Middle of for t: %d tcomm is: %lf, for rank: %d\n", t, tcomm, rank);

        // Swap here to keep track of the previous table (the one we currently have in current should be previous since t increased)
		swap=u_previous;
		u_previous=u_current;
		u_current=swap;


        // If we got data for the previous array from neighbors, add them at the borders
        if (east!=-1)
            for (i = 0; i<local[0]; i++) {
                u_previous[i+1][local[1]+1] = right_receive[i];
            }
        
        if (south!=-1)
            for (j = 0; j < local[1]; j++) {
                u_previous[local[0]+1][j+1] = down_receive[j];
            }


        // If not rank=0 receive t = T for t = T
        // Reset counters and proceed to communication of current timestamp t
        counter_recv = 0;
        counter_send = 0;

        // Measure communication here
        gettimeofday(&tes,NULL);

        if (north != -1) {    
        //  Receive north
            MPI_Irecv(up_receive,local[1],MPI_DOUBLE,north,rank,CART_COMM, &receive_requests[counter_recv]);
            counter_recv++;
        }

        if (west != -1) {
        //   Receive west 
            MPI_Irecv(left_receive,local[0],MPI_DOUBLE,west,rank,CART_COMM, &receive_requests[counter_recv]);
            counter_recv++;
        }

        // Wait for the communication to be completed and copy data to u_current
        MPI_Waitall(counter_recv,receive_requests,MPI_STATUSES_IGNORE);               
        gettimeofday(&tef,NULL);
        tcomm+=(tef.tv_sec-tes.tv_sec)+(tef.tv_usec-tes.tv_usec)*0.000001;
        ...
        // We have all the data neeeded to compute our own part for current timestamp
        // Computation timer
        ...
        // Send the current timestamp data to neighbors so they can calculate their own part
        // Send after computation, t = T for t = T

        // Again copy buffers here so we can measure communication independantly
        //  Send south
        if (south != -1)
            for (i = 0; i < local[1]; i++)
        ...
        // Start measuring communication
        gettimeofday(&tes,NULL);

        //  Send south
        if (south != -1) {
            MPI_Isend(down_send,local[1],MPI_DOUBLE,south,south,CART_COMM, &send_requests[counter_send]);
            counter_send++;
        }
        // Send east 
        if (east != -1) {
            MPI_Isend(right_send,local[0],MPI_DOUBLE,east,east,CART_COMM, &send_requests[counter_send]);
            counter_send++;
        }
        ...
        #endif
        // printf("End of for t: %d tcomm is: %lf, for rank: %d\n", t, tcomm, rank);
    }
    ...
    if (rank==0) {
        #ifdef TEST_CONV
            printf("Gauss X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf CommunicationTime %lf ConvTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,total_time,*comm_time,*conv_time,U[global[0]/2][global[1]/2]);
        #else
            printf("Gauss X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf CommunicationTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,total_time,*comm_time,U[global[0]/2][global[1]/2]);
        #endif
        #ifdef PRINT_RESULTS
            char * s=malloc(50*sizeof(char));
            sprintf(s,"resGaussMPI_%dx%d_%dx%d_iters_%d",global[0],global[1],grid[0],grid[1],t);
